...
/*   Updated: 2024/05/14 11:11:28 by achakour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_move(int new_x, int new_y, t_solong *img)
{
	...
	else
	{
		...
		mlx_put_image_to_window(img->mlx, img->win, img->back_ground, (PXL
				* img->y_player), (PXL * img->x_player));
		img->x_player = new_x;
		img->y_player = new_y;
		mlx_put_image_to_window(img->mlx, img->win, img->player, (PXL * new_y),
			(PXL * new_x));
		mlx_put_image_to_window(img->mlx, img->win, img->wall, (PXL * 1), (PXL
				* 0));
...
void	put_images(t_solong *img)
{
	...
	while (++i < img->x)
	{
		...
		{
			if (map[i][j] == '0')
				mlx_put_image_to_window(img->mlx, img->win, img->back_ground,
					(PXL * j), (PXL * i));
			else if (map[i][j] == '1')
				mlx_put_image_to_window(img->mlx, img->win, img->wall, (PXL
						* j), (PXL * i));
			else if (map[i][j] == 'P')
				mlx_put_image_to_window(img->mlx, img->win, img->player, (PXL
						* j), (PXL * i));
			else if (map[i][j] == 'E')
				mlx_put_image_to_window(img->mlx, img->win, img->c_door, (PXL
						* j), (PXL * i));
			else if (map[i][j] == 'C')
				mlx_put_image_to_window(img->mlx, img->win, img->bomb, (PXL
						* j), (PXL * i));
			else if (map[i][j] == 'V')
				mlx_put_image_to_window(img->mlx, img->win, img->enem, (PXL
						* j), (PXL * i));
		}
	}
}
...
void	move_enemy(int x, int y, t_solong *track)
{
	...
	{
		if ((i % 10000))
			mlx_put_image_to_window(track->mlx, track->win, track->enemy_hit,
				(PXL * y), (PXL * x));
		i += 4;
	}
	mlx_put_image_to_window(track->mlx, track->win, track->enem, (PXL * y), (PXL
			* x));
}
